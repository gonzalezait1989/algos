...
public class SiguienteElementoMayor {
	...
	/**
	 * Complexity of the algorithm is O(n*(m-n)) => which is still O(N^2) because we
	 * are looping an array inside an array. I have choosen to use arrays because we
	 * are not performing any kind of add or get operation or any kind of sorting.
	 * And because the iterations over the inner for decrease on each step there is
	 * no need for dynamic programming or storing previous values.
	 * 
	...
	public int[] getNextMajorElements(int[] inputArray) {
		...
		// If the array is null or empty, I return an empty array (this is arbitrary
		// because there was
		...
		// I initialize the output array, it will always have the same size as the input
		// array
		...
		for (int i = 0; i < inputArray.length; i++) {
			...
			// Initialize the Next Major Element as -1, assuming that we are not going to
			// find any
			// Next Major Element
			int nextMajorElement = -1;

			// Iterates over the following possition of the array to look for a Major
			// element than the current one
			for (int j = i + 1; j < inputArray.length; j++) {
				if (inputArray[j] > currentElement) {
					// If we find a major element, it stores it and gets out of the inner loop
					// because there
